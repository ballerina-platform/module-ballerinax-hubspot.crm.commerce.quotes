...
public isolated client class Client {
    ...
    resource isolated function post batch/read(BatchReadInputSimplePublicObjectId payload, map<string|string[]> headers = {}, *PostCrmV3ObjectsQuotesBatchReadReadQueries queries) returns BatchResponseSimplePublicObject|BatchResponseSimplePublicObjectWithErrors|error {
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function get [string quoteId](map<string|string[]> headers = {}, *GetCrmV3ObjectsQuotesQuoteIdGetByIdQueries queries) returns SimplePublicObjectWithAssociations|error {
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function patch [string quoteId](SimplePublicObjectInput payload, map<string|string[]> headers = {}, *PatchCrmV3ObjectsQuotesQuoteIdUpdateQueries queries) returns SimplePublicObject|error {
        string resourcePath = string `/${getEncodedUri(quoteId)}`;
        map<anydata> headerValues = {...headers};
        if self.apiKeyConfig is ApiKeysConfig {
            headerValues["private-app-legacy"] = self.apiKeyConfig?.privateAppLegacy;
    ...
    # + queries - Queries to be sent with the request 
    # + return - successful operation 
    resource isolated function get .(map<string|string[]> headers = {}, *GetCrmV3ObjectsQuotesGetPageQueries queries) returns CollectionResponseSimplePublicObjectWithAssociationsForwardPaging|error {
